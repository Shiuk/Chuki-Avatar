    ...
    {
        float2 st = frac(poiMesh.uv[0] + float4(0,0,0,0).xy * _Time.x) * float(652.6);
        ...
        switch(float(0))
        {
            ...
            if(float(9.53) > 0)
            {
                randomRotation = randomFloat3WiggleRange(randoPoint, float(72.1));
            }
            else
            {
        ...
        glitterColor *= lerp(1, albedo, float(1));
        #if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)
