Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/2b0b6a6723ee6f741a8f2298f151ef43"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			v2f vert(appdata v)
			{
				...
				{
					float2 udim = 0;
					udim += (v.uv0.xy * (float(0) == 0));
					udim += (v.uv1.xy * (float(0) == 1));
					udim += (v.uv2.xy * (float(0) == 2));
					udim += (v.uv3.xy * (float(0) == 3));
					float isDiscarded = 0;
			...
			void applyUDIMDiscard(in v2f i)
			{
				...
				{
					float2 udim = floor(i.uv[(int)float(0)]);
					float isDiscarded = 0;
					float4 xMask = float4(  (udim.x >= 0 && udim.x < 1),
					(udim.x >= 1 && udim.x < 2),
					(udim.x >= 2 && udim.x < 3),
			...
			{
				poiFragData.alpha = saturate(poiFragData.alpha + float(-0.434));
			}
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				uv = decalUV(float(2), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					lns.x = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.075) * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.427), float(0.187) * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0), float(0.633) * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.075) * blurMap.r, float(0));
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.075) * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.427), float(0.187) * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0), float(0.633) * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(poiLight.lightMap, float(0.5), float(0.075) * blurMap.r, float(0));
				}
				lns = saturate(lns);
				float3 indirectColor = 1;
				if (float4(0.1778884,0,0.3324516,1).a > 0)
				{
					#if defined(PROP_SHADOWCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.1778884,0,0.3324516,1).rgb;
				}
				if (float4(0.4507858,0.5209957,0.693872,1).a > 0)
				{
					#if defined(PROP_SHADOW2NDCOLORTEX) || !defined(OPTIMIZER_ENABLED)
					...
					#endif
					shadow2ndColorTex.rgb = lerp(float3(1, 1, 1), shadow2ndColorTex.rgb, shadow2ndColorTex.a) * float4(0.4507858,0.5209957,0.693872,1).rgb;
					lns.y = float4(0.4507858,0.5209957,0.693872,1).a - lns.y * float4(0.4507858,0.5209957,0.693872,1).a;
				...
				if (float4(0.1778884,0,0.3324516,1).a > 0)
				{
					...
					#endif
					shadow3rdColorTex.rgb = lerp(float3(1, 1, 1), shadow3rdColorTex.rgb, shadow3rdColorTex.a) * float4(0.1778884,0,0.3324516,1).rgb;
					lns.z = float4(0.1778884,0,0.3324516,1).a - lns.z * float4(0.1778884,0,0.3324516,1).a;
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.5));
				float3 finalWrap = directColor + indirectColor;
				if (float(1))
				{
					finalWrap = clamp(finalWrap, _LightingMinLightBrightness, float(1.5));
				}
				else
				{
			...
			{
				float viewDotNormal = abs(dot(poiCam.viewDir, lerp(poiMesh.normals[0], poiMesh.normals[1], float(0.303))));
				...
				float rimWidth = lerp( - .05, 1, float(0.187));
				...
				float rim = 1 - smoothstep(min(float(0.098), rimWidth), rimWidth, viewDotNormal);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float _RimArea_var = (1.0 - dot(lerp(poiMesh.normals[0], poiMesh.normals[1], float(0.303)), poiCam.viewDir));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#if defined(PROP_LIGHTINGAOMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 AOMaps = POI2D_SAMPLER_PAN(_LightingAOMaps, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0));
				poiLight.occlusion = lerp(1, AOMaps.r, float(0)) * lerp(1, AOMaps.g, float(0)) * lerp(1, AOMaps.b, float(0)) * lerp(1, AOMaps.a, float(0));
				#else
				...
				#endif
				#if defined(PROP_LIGHTINGSHADOWMASKS) || !defined(OPTIMIZER_ENABLED)
				float4 ShadowMasks = POI2D_SAMPLER_PAN(_LightingShadowMasks, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0));
				...
				if (float(1))
				{
					poiLight.directColor = min(poiLight.directColor, float(1.5));
					poiLight.indirectColor = min(poiLight.indirectColor, float(1.5));
				}
				if (float(0))
				{
				...
				poiLight.attenuation = attenuation;
				#endif
				poiLight.directColor = float(1) ? min(float(4), _LightColor0.rgb) : _LightColor0.rgb;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				...
				float alphaMask = POI2D_SAMPLER_PAN(_ClippingMask, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0)).r;
				if (float(0))
				{
				...
				if (float(1))
				{
					...
					poiFragData.finalColor *= lerp(poiThemeColor(poiMods, float4(0.03864443,0,0,1).rgb, float(0)), poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), smoothstep(float(0), float(0.15), distance(position, poiCam.worldPos)));
					if (float(0))
					{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.01,1,0.01,1)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0,0,0,0).rgb, float(0)), float4(0,0,0,0).a);
