...
float4 getClearcoatSmoothness(float4 clearcoatMap)
{
	...
	float reflectivity = float(0.789) * clearcoatMap.r;
...
void calculateAndApplyClearCoat(inout float4 finalColor)
{
	...
	float3 creflViewDir = getClearcoatAnisotropicReflectionVector(poiCam.viewDir, poiMesh.binormal, poiMesh.tangent.xyz, poiMesh.normals[0], clearcoatRoughness, float(1));
	...
	float3 vDirectSpecular = 0;
		...
		for (int index = 0; index < 4; index++)
		{
			...
			float3 v0directSpecular = getClearcoatDirectSpecular(clearcoatRoughness, vcndh, max(cvdn, 0.000001), vcndl, saturate(poiLight.vDotLH[index]), clearcoatf0, poiLight.halfDir, poiMesh.tangent, poiMesh.binormal, float(1)) * poiLight.vAttenuation * vcndl * poiLight.vColor[index];
			vDirectSpecular += v0directSpecular;
		}
	#endif
	float3 clearcoatDirectSpecular = getClearcoatDirectSpecular(clearcoatRoughness, cndh, max(cvdn, 0.000001), attenuation, saturate(poiLight.lDotH), clearcoatf0, poiLight.halfDir, poiMesh.tangent, poiMesh.binormal, float(1)) * poiLight.attenuation * attenuation * poiLight.color;
	...
	float3 clearcoat = ((clearcoatDirectSpecular + vDirectSpecular) * clearCoatMap.g * float(0) + clearcoatIndirectSpecular * clearCoatMap.b * float(1)) * clearcoatReflectivity * clearcoatFresnel;
