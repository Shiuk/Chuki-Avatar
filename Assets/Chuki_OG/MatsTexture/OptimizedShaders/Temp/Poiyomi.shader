Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/d2d60c14589b9fc4f929be1bf98d4837"
{
	Properties
	{
 ...
 #define PROP_SET_RIMLIGHTMASK 
			...
			v2f vert(appdata v)
			{
				...
				#ifdef POI_AUDIOLINK
				if (AudioLinkIsAvailable() && float(0))
				{
					...
					{
						ALLocalTranslation = lerp(float4(0,0,0,1), float4(0,0,0,1), audioLinkBands[float(0)]);
					}
					...
					if (any(float4(0,0,0,1)))
					{
						CTALRotation.x = AudioLinkGetChronoTime(float(0), float(0)) * float4(0,0,0,1).x * 360;
						CTALRotation.y = AudioLinkGetChronoTime(float(0), float(0)) * float4(0,0,0,1).y * 360;
						CTALRotation.z = AudioLinkGetChronoTime(float(0), float(0)) * float4(0,0,0,1).z * 360;
					}
					if (any(float4(0,0,0,0)) || any(float4(0,0,0,0)))
					{
						ALScale = lerp(float4(0,0,0,0).xyz + float4(0,0,0,0).w, float4(0,0,0,0).xyz + float4(0,0,0,0).w, audioLinkBands[float(0)]);
					}
					if (any(float4(0,0,0,1)) || any(float4(0,0,0,1)))
					{
						ALWorldTranslation = lerp(float4(0,0,0,1), float4(0,0,0,1), audioLinkBands[float(0)]);
					}
					if (any(float4(0,0,0,1)))
					{
						ALHeight = lerp(float4(0,0,0,1).x, float4(0,0,0,1).y, audioLinkBands[float(0)]);
					}
					if (any(float4(0,0,0,1)))
					{
						ALRoundingAmount = lerp(float4(0,0,0,1).x, float4(0,0,0,1).y, audioLinkBands[float(0)]);
					}
				}
				#endif
				...
				v.vertex = transform(v.vertex, float4(0,0,0,1) + float4(ALLocalTranslation, 0), rotation, float4(1,1,1,1) + float4(ALScale, 0));
				o.normal = UnityObjectToWorldNormal(v.normal);
				#if defined(PROP_VERTEXMANIPULATIONHEIGHTMASK) || !defined(OPTIMIZER_ENABLED)
				float3 heightOffset = (tex2Dlod(_VertexManipulationHeightMask, float4(poiUV(v.uv0, float4(1,1,0,0)) + float4(0,0,0,0) * _Time.x, 0, 0)).r - float(0)) * (float(0) + ALHeight) * o.normal;
				#else
				float3 heightOffset = (float(0) + ALHeight) * o.normal;
				#endif
				if (float(0))
				{
					v.vertex.xz = lerp(v.vertex.xz, normalize(v.vertex.xz) * float(0.2) + v.vertex.xz * float(0), float(0));
				}
				v.vertex.xyz += mul(unity_WorldToObject, float4(0,0,0,1).xyz + ALWorldTranslation + heightOffset).xyz;
				
				if (float(0))
				{
					float divisionAmount = max(float(0.02) + ALRoundingAmount, 0.0000001);
			...
			{
				float viewDotNormal = abs(dot(poiCam.viewDir, lerp(poiMesh.normals[0], poiMesh.normals[1], float(0.329))));
				...
				float rimWidth = lerp( - .05, 1, float(0.159));
				...
				float rim = 1 - smoothstep(min(float(0.065), rimWidth), rimWidth, viewDotNormal);
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float _RimArea_var = (1.0 - dot(lerp(poiMesh.normals[0], poiMesh.normals[1], float(0.329)), poiCam.viewDir));
