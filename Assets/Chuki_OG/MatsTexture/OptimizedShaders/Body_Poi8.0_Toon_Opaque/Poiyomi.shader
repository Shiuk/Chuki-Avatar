Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/f6aa60ecdc9899a48b98bab834d4555e"
{
	Properties
	{
 ...
 #define PROP_ALDECALCOLORMASK 
			...
			void ApplyDetailColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * float(0) * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * float(1.12));
			}
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_REDTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 red = POI2D_SAMPLER_PAN(_RedTexture, _MainTex, poiUV(poiMesh.uv[float(0)], _RedTexture_ST), float4(0,0,0,0));
				#else
				float4 red = 1;
				#endif
				#if defined(PROP_GREENTEXTURE) || !defined(OPTIMIZER_ENABLED)
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DECALMASK) || !defined(OPTIMIZER_ENABLED)
				float4 decalMask = POI2D_SAMPLER_PAN(_DecalMask, _MainTex, poiUV(poiMesh.uv[_DecalMaskUV], _DecalMask_ST), float4(0,0,0,0));
				#else
				float4 decalMask = 1;
				#endif
				...
				decalColor.a *= applyTilingClipping(float(1), uv) * decalMask[float(3)];
				...
				uv = decalUV(_DecalTexture3UV, float4(1,1,0,0), _DecalPosition3, _DecalRotation3 + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
				...
				decalColor.a *= applyTilingClipping(float(0), uv) * decalMask[_Decal3MaskChannel];
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				matcap.rgb *= float(1);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
				matcapMask = POI2D_SAMPLER_PAN(_MatcapMask, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcapMask = 1;
				#endif
				if (float(0))
				...
				blendMatcap(poiLight, poiFragData, _MatcapAdd, float(0), _MatcapMultiply, float(0), float(0), matcap, matcapMask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				matcap2.rgb *= float(1);
				#if defined(PROP_MATCAP2MASK) || !defined(OPTIMIZER_ENABLED)
				matcap2Mask = POI2D_SAMPLER_PAN(_Matcap2Mask, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcap2Mask = 1;
				#endif
				if (float(0))
				...
				blendMatcap(poiLight, poiFragData, _Matcap2Add, float(0), _Matcap2Multiply, float(0), float(0), matcap2, matcap2Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				switch(float(1))
				{
					...
					case 1:
					{
						float3 worldViewUp = normalize(float3(0, 1, 0) - poiCam.viewDir * dot(poiCam.viewDir, float3(0, 1, 0)));
						float3 worldViewRight = normalize(cross(poiCam.viewDir, worldViewUp));
						matcapUV = float2(dot(worldViewRight, normal2), dot(worldViewUp, normal2)) * float(0.43) + 0.5;
						break;
					}
					case 2:
					{
				...
				matcap3.rgb *= float(1);
				#if defined(PROP_MATCAP3MASK) || !defined(OPTIMIZER_ENABLED)
				matcap3Mask = POI2D_SAMPLER_PAN(_Matcap3Mask, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcap3Mask = 1;
				#endif
				if (float(0))
				...
				blendMatcap(poiLight, poiFragData, _Matcap3Add, float(0), _Matcap3Multiply, float(0), float(0), matcap3, matcap3Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
				...
				//ifex float(1)==0
				#ifdef POI_MATCAP3
				float3 normal3 = poiMesh.normals[float(1)];
				matcapUV = 0;
				switch(float(1))
				{
					case 0:
					...
					case 1:
					{
						float3 worldViewUp = normalize(float3(0, 1, 0) - poiCam.viewDir * dot(poiCam.viewDir, float3(0, 1, 0)));
						float3 worldViewRight = normalize(cross(poiCam.viewDir, worldViewUp));
						matcapUV = float2(dot(worldViewRight, normal3), dot(worldViewUp, normal3)) * float(0.43) + 0.5;
						break;
					}
					case 2:
					{
				...
				#if defined(PROP_MATCAP4) || !defined(OPTIMIZER_ENABLED)
				matcap4 = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap4, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap4)) * float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#else
				matcap4 = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0)), float4(1,1,1,1).a);
				#endif
				matcap4.rgb *= float(1);
				#if defined(PROP_MATCAP4MASK) || !defined(OPTIMIZER_ENABLED)
				matcap4Mask = POI2D_SAMPLER_PAN(_Matcap4Mask, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				matcap4Mask = 1;
				#endif
				if (float(0))
				...
				if (float(0))
				{
					matcap4Mask = lerp(matcap4Mask, 1, TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor) * float(1));
				}
				#endif
				poiFragData.alpha *= lerp(1, matcap4.a, matcap4Mask * float(0));
				if (float(0))
				{
					matcap4.rgb = hueShift(matcap4.rgb, float(0) + _Time.x * float(0));
				}
				blendMatcap(poiLight, poiFragData, _Matcap4Add, float(0), _Matcap4Multiply, float(0), float(0), matcap4, matcap4Mask, float(0), float(0)
				#ifdef POI_BLACKLIGHT
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float3 emissionColor = lerp(_ALDecalVolumeColorLow.rgb * float(5.02), _ALDecalVolumeColorMid.rgb * float(5.2), saturate(volumeColorSrc * 2));
				emissionColor = lerp(emissionColor, _ALDecalVolumeColorHigh.rgb * float(5.08), saturate(volumeColorSrc * 2 - 1));
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float flipBookMask = POI2D_SAMPLER_PAN(_FlipbookMask, _MainTex, poiMesh.uv[float(1)], float4(0,0,0,0)).r;
				#else
				...
				float2 uv = frac(poiMesh.uv[float(3)]);
				...
				if (!float(1))
				{
					if (float(0) != 0)
					{
						currentFrame = (_Time.y / (1 / float(0))) % totalFrames;
						if (float(0))
						{
				...
				if (float(0))
				{
					flipBookPixel.rgb = poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
					flipBookPixel.rgb *= poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * float(0) * flipBookMask * flipbookAlpha);
				poiFragData.baseColor = poiFragData.baseColor + flipBookPixel.rgb * float(0) * flipBookMask * flipbookAlpha;
				poiFragData.baseColor = poiFragData.baseColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * flipBookMask * _FlipbookMultiply * flipbookAlpha);
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.6724362,1,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.6724362,1,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.6724362,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)) + (dot(pos, float4(0.5,0,1,0).xyz) * float(3)) + _Time.x * float(0.55)).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(0.5,0,1,0).xyz, float(0.55), float(3), float(10), float(0), pos);
					}
				}
				if (float(0))
				{
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0,0.306108,1,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(0)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0,0.306108,1,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0,0.306108,1,1).rgb, float(0));
				#endif
				if (float(1))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE1) || !defined(OPTIMIZER_ENABLED)
						emissionStrength1 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve1, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)) + (dot(pos1, float4(0,-10,0,0)) * float(20)) + _Time.x * _EmissiveScroll_Velocity1);
						#endif
					}
					else
					{
						emissionStrength1 *= calculateScrollingEmission(float4(0,-10,0,0), _EmissiveScroll_Velocity1, float(20), _EmissiveScroll_Width1, float(0), pos1);
					}
				}
				if (float(0))
				{
					emissionStrength1 *= calculateBlinkingEmission(float(0.35), float(1), float(1.5), float(0));
				}
				applyLumaGradient(poiMods, emissionColor1, float(0), poiLight.nDotV);
				emissionColor1 = hueShift(emissionColor1, frac(_EmissionHueShift1 + float(0) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK1) || !defined(OPTIMIZER_ENABLED)
				float emissionMask1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask1, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0));
				#else
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength1, float4(0,0,0,0), float(3), float4(1,5,0,0), float(3), _EmissionAL1Enabled);
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength1, float(0), float(0), float4(0,0,0,0), _EmissionAL1Enabled);
			...
			float3 applyEmission2(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor2 = POI2D_SAMPLER_PAN(_EmissionMap2, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0,0.3049874,1,1).rgb, float(0));
				}
				else
				{
					emissionColor2 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap2, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(0)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0,0.3049874,1,1).rgb, float(0));
				}
				#else
				emissionColor2 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0,0.3049874,1,1).rgb, float(0));
				#endif
				if (float(0))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE2) || !defined(OPTIMIZER_ENABLED)
						emissionStrength2 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve2, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos2, float4(0,-10,0,0)) * float(20)) + _Time.x * float(10));
						#endif
					}
					else
					{
				...
				applyLumaGradient(poiMods, emissionColor2, float(0), poiLight.nDotV);
				emissionColor2 = hueShift(emissionColor2, frac(_EmissionHueShift2 + float(0) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK2) || !defined(OPTIMIZER_ENABLED)
				float emissionMask2 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask2, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0));
				#else
				...
				applyALEmmissionStrength(poiMods, emissionStrength2, float4(0,10,0,0), float(0), float4(0,0,0,0), float(0), _EmissionAL2Enabled);
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength2, float(1), float(0), float4(0,0.5,0,0), _EmissionAL2Enabled);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(1)], float4(1,1,0,0)), float4(0,0,0,0)), float(1.5));
