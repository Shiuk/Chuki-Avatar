Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/18cc7987de4f75042b867871b6d26854"
{
	Properties
	{
 ...
 #define PROP_EMISSIONSCROLLINGCURVE 
			...
			v2f vert(appdata v)
			{
				...
				if(float(0) == 0) // Discard Vertices instead of just pixels
				{
					float2 udim = 0;
					udim += (v.uv0.xy * (float(2) == 0));
					udim += (v.uv1.xy * (float(2) == 1));
					udim += (v.uv2.xy * (float(2) == 2));
					udim += (v.uv3.xy * (float(2) == 3));
					float isDiscarded = 0;
					float4 xMask = float4(  (udim.x >= 0 && udim.x < 1),
					(udim.x >= 1 && udim.x < 2),
					(udim.x >= 2 && udim.x < 3),
					(udim.x >= 3 && udim.x < 4));
					isDiscarded += (udim.y >= 0 && udim.y < 1) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
					...
					isDiscarded += (udim.y >= 3 && udim.y < 4) * dot(float4(float(0), float(0), float(0), float(0)), xMask);
				...
				float3 outlineNormal = float(1) ? o.normal : v.normal;
				if (float(0) == 1)
				{
			...
			{
				if(float(0) == 1) // Don't run if in vertex mode
				{
					float2 udim = floor(i.uv[(int)float(2)]);
					float isDiscarded = 0;
					float4 xMask = float4(  (udim.x >= 0 && udim.x < 1),
					(udim.x >= 1 && udim.x < 2),
					(udim.x >= 2 && udim.x < 3),
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.2);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.2);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.2) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				shadowStrength = lerp(0, shadowStrength, float(1));
				#endif
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.2));
				...
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.2));
				#endif
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.2) * poiLight.shadowMask);
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(1.5);
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.3215686,0.01833714,0.01176471,0).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(10)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.3215686,0.01833714,0.01176471,0).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.3215686,0.01833714,0.01176471,0).rgb, float(0));
				#endif
				if (float(1))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos, float4(0.5,0,1,10).xyz) * float(3)) + _Time.x * float(0.5)).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(0.5,0,1,10).xyz, float(0.5), float(3), float(10), float(0), pos);
					}
				}
				if (float(0))
				{
					emissionStrength0 *= calculateBlinkingEmission(float(1), float(1), float(0), float(0));
				}
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.3215686,0.04855409,0.01176471,1).rgb, float(0));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.3215686,0.04855409,0.01176471,1).rgb, float(0));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.3215686,0.04855409,0.01176471,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				#endif
				applyALEmmissionStrength(poiMods, emissionStrength1, float4(0.5,1.5,0,0), float(2), float4(0,1,0,0), float(2), float(1));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength1, float(0), float(0), float4(0,0,0,0), float(1));
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.009136911,0,1).rgb, float(0)), float4(1,0.009136911,0,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.009136911,0,1).rgb, float(0)), float4(1,0.009136911,0,1).a);
				#endif
				
				if (float(1))
				{
					rimColor.rgb = hueShift(rimColor.rgb, _RimHueShift + _Time.x * float(0));
				}
				rimWidth = max(lerp(rimWidth, rimWidth * lerp(0, 1, poiLight.lightMap - _ShadowMixThreshold) * _ShadowMixWidthMod, _ShadowMix), 0);
				float rim = 1 - smoothstep(min(float(0.245), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(1,0.009136911,0,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.009136911,0,1).rgb, float(0)));
				...
				if (float(1))
				{
					_RimLight_var = hueShift(_RimLight_var, _RimHueShift + _Time.x * float(0));
				}
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(652.6);
				...
				switch(float(0))
				{
					...
					if (float(9.53) > 0)
					{
						randomRotation = randomFloat3WiggleRange(randoPoint, float(72.1), float(9.53));
					}
					else
					{
						randomRotation = randomFloat3Range(randoPoint, float(72.1));
					}
					float3 glitterReflectionDirection = normalize(mul(poiRotationMatrixFromAngles(randomRotation), norm));
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.836)), poiCam.viewDir)), float(601.41)), 0);
					finalGlitter *= glitterAlpha;
					break;
					case 1:
					float offset = random(randoPoint);
					float brightness = sin((_Time.x + offset) * float(9.53)) * float(20) - (float(20) - 1);
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(601.41)), 0);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.06480327,0.006450434,0.003346534,0.9294118).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.06480327,0.006450434,0.003346534,0.9294118).a;
				//ifex float(1)==0
 ...
 #define PROP_EMISSIONSCROLLINGCURVE 
			...
			{
				clip(float(1) - 0.01);
				float OutlineMask = tex2D(_OutlineMask, TRANSFORM_TEX(poiMesh.uv[float(1)], _OutlineMask) + _Time.x * float4(0,0,0,0)).r;
				if (float(0))
				{
					float lineWidth = float(0.05);
					#ifdef POI_AUDIOLINK
				...
				poiFragData.baseColor = lerp(col, col * poiFragData.baseColor, float(0.233));
				if (float(1) == 2)
				{
				...
				if (float(0))
				{
					poiFragData.baseColor = hueShift(poiFragData.baseColor, float(1) +float(1) * _Time.x);
				}
				float emission = float(2.08);
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
